...
{
  cout << "Generate Parallel Simulator" << endl;
...
void CIRCUIT::combineFilesToOutput()
{
	...
	ifs.open("./sim/evaluate");
	if(ifs.is_open()){
		ofs << ifs.rdbuf();
		ifs.close();
	}
	ifs.open("./sim/printIO");
	if(ifs.is_open()){
		ofs << ifs.rdbuf();
		ifs.close();
	}

	system("rm -f ./sim/header");
	system("rm -f ./sim/main");
	system("rm -f ./sim/evaluate");
	system("rm -f ./sim/printIO");
}
